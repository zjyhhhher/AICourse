...
def build_dataset(is_train, args):
    ...
    if is_test:
        ...
        for dataset in args.dataset_list:
            ...
            transform_4 = transforms.Compose(transform_list_4)
            dataset_1 = ImageFolder(root, transform=transform_1)
            dataset_2 = ImageFolder(root, transform=transform_2)
            dataset_3 = ImageFolder(root, transform=transform_3)
            dataset_4 = ImageFolder(root, transform=transform_4)
            dataset = merge_datasets(dataset,original_dataset) 
            dataset = merge_datasets(dataset, dataset_1)
            dataset = merge_datasets(dataset, dataset_2)
            dataset = merge_datasets(dataset, dataset_3)
            dataset = merge_datasets(dataset, dataset_4)
            dataset_list.append(dataset)  
            nb_classes += len(dataset.classes)
