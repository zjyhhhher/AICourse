...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    print_freq = 10
    print("OK")
    print(type(metric_logger.log_every(data_loader, print_freq, header)))
    print("hhhh")

    for data in metric_logger.log_every(data_loader, print_freq, header):
        #print(data)
        #print("1")
        sample, target_or_star, flag, dataset_ids = data
        #print("OK@")
        #print(f"FUCK YOU TYPE{type(samples)}")
        samples = samples.to(device, non_blocking=True)
        target_or_star = target_or_star.to(device, non_blocking=True)
        #targets = targets.to(device, non_blocking=True)
        #print(f"TARGET:{targets}")
        if args.bce_loss:
            target_or_star = target_or_star.gt(0.0).type(targets.dtype)
        with torch.cuda.amp.autocast():
            if flag:
                outputs = model(samples, dataset_ids)
                output_star = model(target_or_star,dataset_ids)
            else:
                outputs = model(samples, dataset_ids)
            ...
            if flag:
                mse = nn.MSELoss()
                loss = mse(outputs, output_star)
            else:
                loss = criterion(outputs, target_or_star)
...
def test(data_loader, model, device, dataset_id=None, num_classes_list=None, know_dataset=True):
    ...
    for data in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            #output = model(images) # teacher 只有self和x两个参数
        file_ids = data[-1].tolist()
