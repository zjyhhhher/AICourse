...
from timm.models import regnetx_160 #新增的
import timm.models
...
from engine_cjm import train_one_epoch, evaluate, test
...
from ViT_co import ViT_CoTrain
...
class CustomClassifier(torch.nn.Module):
    def __init__(self, model, input_dim, output_dim, 
        ...
        self.layers = torch.nn.Sequential(
            torch.nn.Linear(input_dim, hidden_dim),  # 第一层线性层
            torch.nn.ReLU(),  # 激活函数（例如ReLU）
            torch.nn.Linear(hidden_dim, output_dim)  # 第二层线性层
        )

    def forward(self, img, dataset_id=None):
        # TODO: how to leverage dataset_source in training and inference stage?
        pdtype = img.dtype
        #print("BEFORE FORWARD FEATURE")
        #print(type(self.backbone).__name__)
        feature = self.backbone.forward_features(img).to(pdtype)
        #print(f"FREATURE IS {feature}")
        #print("AFTER FORWARD FEATURE")
        #print(type(feature))
        #print(len(feature))
...
def get_args_parser():
    ...
    parser.add_argument('--known_data_source', action='store_false', dest='known_data_source', default=True)
    parser.add_argument('--unknown_data_source', action='store_true', dest='known_data_source', default=True)
...
def main(args):
    ...
    model = ViT_CoTrain(patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), num_labels=654, lost_rate=0.2)
    #model.load_state_dict(torch.load('regnety_160-a5fe301d.pth'))

    #model =   Models.DistilledVisionTransformer(
