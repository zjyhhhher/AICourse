...
def logits_CE(p,q):
    ...
    log_q = torch.log(q)
    vector = (-p*log_q).sum(dim=1)
    #print(f"LOGITS_CE:{(vector.sum(dim=0))/len(vector)}")
    return (vector.sum(dim=0))/len(vector)
...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    for data in metric_logger.log_every(data_loader, print_freq, header):
        ...
        #print(f"FUCK YOU TYPE{type(samples)}")
...
#            outputs = model(samples, dataset_ids)
            ...
            outputs_1 = model(samples,0)
            #print(f'OUTPUTS_1:{outputs_1}')
            outputs_2 = model(samples,0)
            #print(f'OUTPUTS_2:{outputs_2}')
            ...
            softmax = nn.Softmax(dim=1)
            loss = (0.9/2)*(criterion(outputs_1, targets)+criterion(outputs_2, targets)) + (0.1/2)*(logits_CE(outputs_1, outputs_2)+logits_CE(outputs_2, outputs_1))
...
def evaluate(data_loader, model, device, dataset_id=None, dump_result=False):
    ...
    for data in metric_logger.log_every(data_loader, 10, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            #output = model(images, dataset_id) # student的model可能还是会用到dataset_id
            output = model(images) # teacher的model只有一个参数
...
def test(data_loader, model, device, dataset_id=None, num_classes_list=None, know_dataset=True):
    ...
    for data in metric_logger.log_every(data_loader, 10, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            #output = model(images, dataset_id) # student的model要用
            output = model(images,1) # teacher 只有self和x两个参数
        file_ids = data[-1].tolist()
